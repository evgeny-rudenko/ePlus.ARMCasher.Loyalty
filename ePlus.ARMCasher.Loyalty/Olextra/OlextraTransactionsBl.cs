...
namespace ePlus.ARMCasher.Loyalty.Olextra
{
	public class OlextraTransactionsBl
	{
		...
		public OlextraTransactionsBl()
		{
		}

		public IEnumerable<CHEQUE_ITEM_TRANSACTION> GetListUnconfirmed()
		{
			string str = string.Format("EXEC USP_CHEQUE_ITEM_TRANSACTION_OLEXTRA_LOAD", new object[0]);
		...
		{
			string str = "#CHEQUE_ITEM_TRANSACTION_OLEXTRA";
					...
					using (DataTable dataTable = this.loader.CreateDataTable(transactions))
					{
						using (SqlCommand sqlCommand = sqlConnection.CreateCommand())
						{
							using (SqlCommand sqlCommand1 = sqlConnection.CreateCommand())
							{
								...
								sqlCommand1.CommandText = "EXEC USP_CHEQUE_ITEM_TRANSACTION_OLEXTRA_SAVE";
